...
treatmentlist<-c("Dongguan","Zhongshan","Foshan","Beihai","Nanning","Nanchang" ,"Xiamen","Taizhou","Ningbo","Guangzhou" ,"Huizhou","Hangzhou","Liuzhou","Shantou","Jiangmen","Heyuan","Quanzhou","Haikou" ,"Shenzhen","Wenzhou","Huzhou","Zhuhai","Fuzhou","Shaoxing","Zhaoqing","Zhoushan","Quzhou","Jinhua","Shaoguan" ,"Sanya","Jieyang" ,"Meizhou","Shanwei","Zhanjiang" ,"Chaozhou","Maoming" ,"Yangjiang")
...
for (n in 1:length(startdate)){
  ...
  for (m in 1:length(treatmentlist)){
    ...
    test3 <- test2 %>% 
      filter(test2$ID == "Dongguan"| test2$ID == "Zhongshan" | test2$ID == "Foshan"| test2$ID == "Beihai"|
...
write.csv(data_exp,"in_place_placebo.csv",row.names = FALSE)